  ...
  return (
    <>
      <Card className="w-full border-none">
        ...
        <CardContent>
          <DocumentChooserBar activeDocClass={activeDocClass} processing={processing} chooseDocClass={chooseDocClass} />
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {previewUrl && file ? (
                <div className="flex flex-col items-center space-y-2">
                  {file.type.startsWith("image/") ? (
                    <Image src={previewUrl || "/placeholder.svg"} alt="Selected file" width={200} height={200} />
                  ) : (
                    <File className="w-16 h-16 text-primary" />
                  )}
                  <span className="text-sm text-center break-all max-w-[200px]">{file.name}</span>
                </div>
              ) : (
            ...
            <div className="flex justify-end space-x-2">
              <Button type="button" variant="outline" onClick={() => router.push(`/project-view/${projectId}`)}>
                Cancel
              </Button>
