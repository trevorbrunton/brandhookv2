...
import { ArrowRight, UserRoundIcon as UserRoundPen } from "lucide-react";
...
export const SettingsDialog = ({ userId }: SettingsDialogProps) => {
  ...
  const { data: userRecord, error } = useQuery({
    queryKey: ["user", userId],
    queryFn: async () => {
      return fetchUserByUserId(userId);
    },
    enabled: open, // This ensures the query only runs when the dialog is open
  });
